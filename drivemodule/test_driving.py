...
trn_bds = BatchifiedDataset(df_small_trn, bs, seq_len, trn_tfms)
val_bds = BatchifiedDataset(df_small_val, bs, seq_len, trn_tfms)
...
opt_fn = lambda *args, **kwargs: optim.SGD(*args, momentum=0.9, **kwargs) # optimizer = opt_fn(trainable_params(model), lr=1e-1)
#optimizer = optim.SGD(trainable_params(model),lr=1e-1, momentum=0.9)
criterion = F.l1_loss
