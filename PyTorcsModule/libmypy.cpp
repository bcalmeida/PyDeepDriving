...
int torcs_run(int argc, char** argv) {
    ...
    while (1) {
        ...
        if (shared->written == 1) { // the new image data is ready to be read
            ...
            /////// MOCKED DEEP LEARNING DETECTION
            angle=true_angle;

            toMarking_L=true_toMarking_L;
            toMarking_M=true_toMarking_M;
            toMarking_R=true_toMarking_R;

            dist_L=true_dist_L;
            dist_R=true_dist_R;

            toMarking_LL=true_toMarking_LL;
            toMarking_ML=true_toMarking_ML;
            toMarking_MR=true_toMarking_MR;
            toMarking_RR=true_toMarking_RR;

            dist_LL=true_dist_LL;
            dist_MM=true_dist_MM;
            dist_RR=true_dist_RR;

            if (true_fast>0.5) fast=1;
            else fast=0;
